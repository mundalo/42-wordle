		...
		while (index < (presentLetters.length || correctLetters.length))
		{
			...
			if (counter > 1)
			{
				...
				while (counterCorrectList < counter)
				{
					...
					counterCorrectList = amountOfLetterInStr(correctLetters, presentLetters[index]);
				}
			}
...
function	removeWordsBelowCount(remainingWords, letter, count)
{
	let filtered = [];
...
function	removeWordsNotEqualCount(remainingWords, letter, count)
{
	let filtered = [];
...
function	removeBasedOnColor(remainingWords, correctLetters, tile, curTile)
{
	...
	if (amountOfLetters > 1)
		remainingWords = removeWordsBelowCount(remainingWords, letter, amountOfLetters);
	if (color == 'y')
	{
		...
		{
			remainingWords = removeWordsNotEqualCount(remainingWords, letter, amountOfLetters);
